...
const client_email = process.env.client_email || require('../../etc/config.json').client_email;
const private_key = process.env.private_key || require('../../etc/config.json').private_key;

console.log('client email and private keys: ')
...
const uploadUserFile = (fileName, fileContent, userEmail, uploadTypeProject=true) => {
    return new Promise(async(resolve, reject) => {
        ...
        try{
            ...
            console.log('trying to search here');
